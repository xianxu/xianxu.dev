...
published: false
excerpt: "The key role of human in agentic coding is to set up what I would call the deterministic shell in more places such that human intention can be enforced. Agent skill is the main way current generation of harness use to stitch together static set of prose, potentially with embedded scripts. Here, I demonstrate two patterns I used in ariadne to make such prose fragments easier to maintain and dynamic."
tags:
  - ai
  - tech
---

In the context of agentic coding, in the end of the day, you assemble some system prompts to prime how an LLM based intelligent agent work in a session. **That's the mechanism for using the LLM -- the new stochastic computer.** And the current generation of LLM based agents use [agent skill](https://agentskills.io/home) convention for constructing such prompt (aka context). The convention works roughly as follows: there is a folder with the name of the skill, and within that folder a file named `SKILL.md` serves as the entry point. `SKILL.md` contains a front-matter formatted meta-data section at the top. The body is prose describing what the agent is supposed to do. One front-matter field is of particular interest: `description`, which is always loaded into the agent's context at startup. The rest of the document is pulled into context later in the session -- by the agent's own determination, based on that description.
...
## Problem one: how to keep script in sync with SKILL.md
...
2. put most, if not all, scripts in the skill folder as subcommand of that Go binary.
3. write regular tests of that binary.
...
You can check out this pattern in [sdlc skill](https://github.com/xianxu/ariadne/blob/main/construct/local/sdlc/SKILL.md) in ariadne. The `sdlc`'s SKILL.md is very short, basically of point 4. above. All the real prose are in the `sdlc` binary's [help text](https://github.com/xianxu/ariadne/tree/main/cmd/sdlc/helptext). Go's embed functionality means the help text stays in plain markdown files on disk, just bundled into the binary at compile time.
...
[^programming]: using quotes here as this is not the traditional programming; it's a combination of agent instructions, repo architecture, and some code but writing by agents.
