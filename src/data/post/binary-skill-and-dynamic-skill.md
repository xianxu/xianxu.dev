...
1. 🤖<pick a stronger language>{Pick a stronger language} than just shell scripts. 🤖<I picked Go, for its static typing, fast compilation and simple release packaging>{I picked Go for its static typing, fast compilation, and simple release packaging} (one single small binary).
2. 🤖<put most, if not all, scripts in the skill folder as subcommand>{Put most, if not all, scripts in the skill folder as subcommands} of that Go binary.
